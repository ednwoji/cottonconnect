...
public interface FaqResponseRepo extends JpaRepository<FaqQueryResponse, Long>{
	...
	@Query(value = "SELECT count(id),q.created_date FROM public.faq_query q where q.created_date BETWEEN :startDate AND :endDate group by q.created_date order by q.created_date asc",nativeQuery = true)
	public List<Object[]> getQueriesReceived(Date startDate, Date endDate);
	
	@Query(value = "SELECT count(r.id),q.created_date FROM public.faq_query q left join faq_response r on r.faq_query_id = q.id where q.created_date BETWEEN :startDate AND :endDate group by q.created_date order by q.created_date asc",nativeQuery = true)
